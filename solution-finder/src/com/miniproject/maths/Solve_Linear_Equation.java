...
{
    public static String lineqn(String length , String strInput)
    {
    	try{
        char []var = {'x', 'y', 'z', 'w'};
        ...
        String[][] coEff=new String[n][n];
        String resultString="";
        for(int i=0;i<lineInput.length;i++){
        	coEff[i]=lineInput[i].split(" ");
        }
        
//        if((n*n)+n != coEff.length){
//        	return "Invalid input!! Please enter valid input";
//        }
        ...
        for(int i=0; i<n; i++)
        {
            for(int j=0; j<n; j++)
            {
                mat[i][j] = Double.parseDouble(coEff[i][j]);
            }
            constants[i][0] = Double.parseDouble(coEff[i][n]);
        }
        ...
        for(int i=0; i<n; i++)
        {
        	resultString = resultString+"<br>"+result[i][0];
    ...
    public static double[][] invert(double a[][]) throws Exception
    {
        int n = a.length;
...
// elimination.  Here index[] stores pivoting order.
    ...
    public static void gaussian(double a[][], int index[]) throws Exception
    {
        int n = index.length;
