...
import com.miniproject.DAA.Fra_Knapsack_code;

public class Fractional_Knapsack_Frame {
	JFrame f;
	JTextField tf1,tf2,tf3;
	JLabel label1,label2,label3,statusLabel,resultLabel;
	JButton solve;
	public void f_Knapsack(){
		f=new JFrame("Fractional Knapsack");
		...
		solve.addActionListener(new CustomKnapsackAlgo());
		tf3.addKeyListener(new CustomKeyListener());
	}
	class CustomKnapsackAlgo implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if(StringUtils.isBlank(weight)||StringUtils.isBlank(profit)||StringUtils.isBlank(knap_weight)){
				...
				Fra_Knapsack_code fra=new Fra_Knapsack_code();
				String result=fra.conversion(weight,profit,knap_weight);
